...
select * from employees
where firstname or lastname = 'manager';
...
Select departmentid, count(*) as EmployeesJoined2024 from employees
...
select departmentid, count(*) as No_of_Employees from employees
where salary between 50000 and 70000
group by departmentid;

-- Situation -10: For diversity analysis, count the number of employees whose names start with 
-- each letter of the alphabet.
SELECT LEFT(FirstName, 1) AS StartingLetter, COUNT(*) AS EmployeeCount FROM Employees
...
-- in each city. 
select departmentid, Location, sum(AnnualBudget) as total_budget_allocated from departments
group by departmentid;


-- Situation -13: For leadership tracking, count the number of departments headed by each 
-- manager. 
select HeadOfDepartment, count(DepartmentName) as departments_Headed from departments
group by HeadOfDepartment;


-- Situation -14: To plan budget allocation, calculate the average budget of departments in each 
-- city. 
select location, count(DepartmentName) as average_budget from departments
group by location;


-- Situation -15: For funding efficiency, find the maximum and minimum budgets among 
-- departments in each city.
select location, max(DepartmentName) as Maximum_budget , min(DepartmentName) as Minimum_budget from departments
group by location;
...
-- name and a comma. 
Select concat ('Hello ', firstname, ' ', lastname, ',' ) as department_address from employees;
...
SELECT CONCAT_WS(' ext ', PhoneNumber, Extension) AS ContactNumber FROM Employees;
...
add Current_Project Varchar(50); 
select concat(JobTitle,'  ', Current_Project) as employees_work_status  from employees;
...
where Email like '%@example.com' and dateofjoining > '2023-01-01';
...
delete from employees
where year(dateofjoining) < 2020 and Current_Project is null;
...
delete from employees
where region = 'North' and YEAR(employment_endDate) < 2023 ;
...
SELECT EmployeeID, MaritalStatus FROM Employees;
alter table employees 
...
SELECT EmployeeID, DateofBirth FROM Employees;
alter table employees 
